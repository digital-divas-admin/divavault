...
Python-based platform scanning backend. Has its own `CLAUDE.md` at `apps/scanner/CLAUDE.md` with full documentation.

The scanner shares the same Supabase PostgreSQL database as the frontend. The admin dashboard (Command Center) lives in the Next.js app at `/admin/scanner` — queries in `src/lib/scanner-command-queries.ts`, components in `src/components/admin/scanner/`.
