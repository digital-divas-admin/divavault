...
import { seedContributorSchema } from "@/lib/scanner-test-validators";
...
export async function POST(request: NextRequest) {
  ...
  const parsed = seedContributorSchema.safeParse(body);
  if (!parsed.success) {
  ...
  try {
    const res = await fetch(`${scannerUrl}/admin/seed/contributor`, {
      method: "POST",
      headers: {
